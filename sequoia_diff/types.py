...
@dataclass
class MappingDict:
    src_to_dst: dict[Node, Node] = field(
        default_factory=lambda: defaultdict(lambda: None)
    )
    dst_to_src: dict[Node, Node] = field(
        default_factory=lambda: defaultdict(lambda: None)
    )
    ...
    def has_unmapped_src_children(self, node: Node):
        ...
        for child in gen:
            if child is None:
                break
            if child not in self.src_to_dst:
                return True
    ...
    def has_unmapped_dst_children(self, node: Node):
        ...
        for child in gen:
            if child is None:
                break
            if child not in self.dst_to_src:
                return True
...
class NodePriorityQueue:
    ...
    def synchronize(self, other: "NodePriorityQueue") -> bool:
        while not (self.empty() or other.empty()) and (
            self.curr_prio() != other.curr_prio()
        ):
